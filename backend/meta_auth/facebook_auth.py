...
def save_facebook_auth(access_token, pages):
    try:
        ...
        if pages:
            if existing_auth.data:
                # Get existing pages
                existing_pages = supabase.table('facebook_pages').select('page_id').eq('auth_id', auth_id).execute()
                existing_page_ids = [page['page_id'] for page in existing_pages.data] if existing_pages.data else []
                
                # Filter out pages that already exist
                new_pages = [page for page in pages if page['id'] not in existing_page_ids]
                ...
                if new_pages:
                    pages_data = [{
                        ...
                        'profile_picture': download_and_encode_image(page['profilePicture']),
...
def facebook_auth():
    ...
    try:
        ...
        response = save_facebook_auth(long_lived_token, pages)

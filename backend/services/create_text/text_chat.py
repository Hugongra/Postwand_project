...
from models.chat.controller import ChatController, StructuredPost

PROVIDER = "openai"

chat = ChatController.chat_with_memory(PROVIDER)


def _load_system_prompt(mode):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(current_dir, f"system_prompt_{mode}.txt")
...
def send_chat_message(user_id):
    try:
        ...
        if check_token_limit(user_id, "openai"):
            return {"success": False, "error": "Monthly token limit exceeded"}, 429

        system_prompt = _load_system_prompt(mode)

        if mode == "write_post" and platforms:
            system_prompt += (
                "\n\nIMPORTANT: Create exactly ONE post for each of these platforms: "
                f"{', '.join(platforms)}. Each post should be optimized specifically "
                "for that platform's audience and format."
            )
        ...
        if mode == "write_post":
            ...
            if tokens:
                update_token_usage(user_id, tokens, "openai")
...
def text_action(user_id):
    try:
        ...
        simple_chat = ChatController.chat_base(PROVIDER)
        system_prompt = (
            f"You are a {platform} content expert. Return only the modified post "
            "content, no explanations. IMPORTANT: Always respond in the same language "
            "that the user writes in."
        )

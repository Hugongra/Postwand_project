...
def save_youtube_channels(user_id, access_token, refresh_token, channels):
    """Save YouTube channel data to database"""
    try:
        print(f"Saving {len(channels)} YouTube channels")
        
        # Get existing channels for this user
        existing_channels = supabase.table('youtube_channels').select('account_id').eq('user_id', user_id).execute()
        existing_channel_ids = [ch['account_id'] for ch in existing_channels.data] if existing_channels.data else []
        ...
        if existing_channels.data:
            for existing_channel in existing_channels.data:
                ...
                }).eq('user_id', user_id).eq('account_id', existing_channel['account_id']).execute()
        ...
        if new_channels:
            ...
            for channel in new_channels:
                ...
                if channel.get('profile_picture'):
                    try:
                        profile_pic = image_to_data_uri(channel['profile_picture'])
                    except Exception:
                        profile_pic = None
            ...
            try:
                ...
                return {'error': f'Failed to save YouTube channels: {str(e)}'}, 500
        if not new_channels:
...
def youtube_auth(user_id):
    ...
    if request.method == 'GET':
        ...
        if error:
            ...
            return {"error": f"OAuth error: {error}"}, 400
    ...
    try:
        if auth_code:
            ...
            if not redirect_uri:
                return {"error": "Redirect URI is required"}, 400
            ...
            return {"error": "No access token or authorization code provided"}, 400
        ...
        response = save_youtube_channels(user_id, access_token, refresh_token, channels)

...
const CustomDropdown = ({ value, onChange, placeholder, options, width, placeholderWidth }) => {
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block text-left">
      ...
      {/* Dropdown List */}
      {open && (
        <div className={`absolute mt-2 p-1 bg-white rounded-md border z-50 ${width}`}>
          {options.map(option => (
            <div
              key={option.value}
              onClick={() => {
              ...
              className="p-2 rounded-sm hover:bg-gray-100 cursor-pointer flex items-center gap-3"
            >
              {option.icon}

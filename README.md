...
- **Download demo video (Git LFS)**: [`POSTWAND DEMO.mp4`](POSTWAND%20DEMO.mp4?raw=1)
- **Why it doesn’t preview on GitHub**: GitHub often refuses to render very large video files in the UI (even when stored via Git LFS) and will show “View raw / can’t show files that are this big”. The link above is for **direct download**.
- **Want an embedded playable demo?** Upload the video to YouTube/Vimeo (or add a small preview clip under ~10–20 MB) and link it here.
